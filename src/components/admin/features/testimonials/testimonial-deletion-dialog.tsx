...
import { Testimonial, TestimonialApiResponse } from "@/types/admin/testimonial";
import { cn } from "@/utils/shared/cn";

interface TestimonialDeletionDialogProperties {
  testimonial: Testimonial;
  className?: string;
}

export default function TestimonialDeletionDialog({
  testimonial,
  className,
}: TestimonialDeletionDialogProperties) {
  ...
  const handleDeleteConfirm = () => {
    request.del(ENDPOINTS.ADMIN.TESTIMONIALS.DESTROY(testimonial.id), {
      tanstack: {
        invalidateQueries: ["admin-testimonials"],
        mutationOptions: {
          onSuccess: () => {
            // Optimistic cache update - remove deleted testimonial from list
            request.queryCache.setQueryData<TestimonialApiResponse>(
              ["admin-testimonials"],
              (previous) => {
                ...
                return {
                  ...previous,
                  data: previous.data.filter((t) => t.id !== testimonial.id),
