...
import { useRequest } from "@/hooks/use-request"; // your existing hook
import { Faq } from "@/types/public/program";

export default function ProgramsFAQ() {
  // Fetch all program FAQs from the API
  const { data: faqsResponse, loading, isFetching } = useRequest({
    url: ENDPOINTS.PUBLIC.PROGRAM_FAQS, 
    queryKey: ["programs-faqs"],
    staleTime: 1000 * 60 * 5,
  });

  const faqs = faqsResponse?.data as Faq[] ?? [];
    ...
    <section className="relative overflow-hidden bg-white py-24">
      ...
      <div className="relative mx-auto max-w-4xl px-6 lg:px-8">
        ...
        <div className="mb-16 text-center">
          ...
          <h2 className="mb-6 text-3xl font-bold text-slate-900 sm:text-4xl lg:text-5xl" style={{ fontFamily: "Poppins, sans-serif" }}>
            Frequently Asked{" "}
            <span className="bg-gradient-to-r from-[#35bec5] via-[#4bc4db] to-[#0c96c4] bg-clip-text text-transparent">
              Questions
            </span>
          </h2>
          <p className="mx-auto max-w-2xl text-lg leading-relaxed text-slate-600" style={{ fontFamily: "Inter, sans-serif" }}>
          ...
          {loading || isFetching
            ...
            : faqs.map((faq: Faq, index) => (
                <Collapsible key={index} className="group">
                  <div className="rounded-2xl border border-slate-200 bg-white shadow-sm transition-all duration-300 hover:shadow-md" data-aos="fade-up" data-aos-delay={`${index * 100}`}>
                    <CollapsibleTrigger className="flex w-full items-center justify-between p-6 text-left transition-colors duration-300 hover:bg-slate-50">
                      <h3 className="text-lg font-semibold text-slate-900 group-hover:text-[#35bec5] transition-colors duration-300" style={{ fontFamily: "Poppins, sans-serif" }}>
                        {faq.question}
                      </h3>
