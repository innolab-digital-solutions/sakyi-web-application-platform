...
const breadcrumbMap = new Map(Object.entries(breadcrumbConfig));
...
export const getBreadcrumbsForPath = (pathname: string): BreadcrumbItem[] => {
  ...
  const exactMatch = getBreadcrumbConfig(sanitizedPath);
  if (exactMatch) {
    return exactMatch;
  }
  ...
  for (const route of dynamicRoutes) {
    if (matchesDynamicRoute(sanitizedPath, route)) {
      ...
      const routeConfig = getBreadcrumbConfig(route);
 ...
 */
export const addBreadcrumbConfig = (path: string, breadcrumbs: BreadcrumbItem[]) => {
 ...
 */
const getBreadcrumbConfig = (path: string): BreadcrumbItem[] | undefined => {
